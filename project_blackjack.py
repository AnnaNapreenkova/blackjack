...
class Game(object):
        ...
        def calc(self, dealer, player):            
            blackjack = set(['A','10'] or ['A','K'] or ['A','Q'] or ['A','J'])            
            if set(dealer.hand) == blackjack:
                if set(player.hand) != blackjack:
                    self.player.score = -1                    
                else:
                    self.player.score = 0                  
...
class Player(Dealer):
    ...
    def place_bet(self, amount=10):    
        #amount = input("Enter amount of your bet:")
        self.funds -= amount
        self.bet += amount
